...
function AdminDisputeDetail() {
    ...
    try {
      ...
      switch (action) {
        case 'release_to_seller':
          ...
          data.notes = adminNote || 'Dispute resolved in favor of seller';
          break;
        case 'refund_buyer':
          endpoint = `${API}/admin/transactions/${transaction?.transaction_id}/refund`;
          data.reason = adminNote || 'Dispute resolved in favor of buyer';
          break;
        case 'request_buyer_info':
          endpoint = `${API}/admin/disputes/${disputeId}`;
          method = 'patch';
          data = { status: 'awaiting_buyer_response', admin_notes: adminNote || 'Additional information requested from buyer' };
          break;
        case 'request_seller_info':
          endpoint = `${API}/admin/disputes/${disputeId}`;
          method = 'patch';
          data = { status: 'awaiting_seller_response', admin_notes: adminNote || 'Additional information requested from seller' };
          break;
  ...
  const getDisputePhotoUrl = (photo) => {
    ...
    if (photo.startsWith('http')) return photo;
    const cleanPath = photo.replace(/^\/+/, '');
    return `${BACKEND_URL}/uploads/${cleanPath.includes('/') ? cleanPath : `disputes/${cleanPath}`}`;
  };
    ...
    <div className="min-h-screen" style={{ backgroundColor: COLORS.section }}>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <Breadcrumbs items={[
          ...
          { label: 'Disputes', href: '/admin?tab=disputes' },
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold" style={{ color: COLORS.text }} data-testid="dispute-title">
          ...
          <Badge style={{ backgroundColor: getDisputeStatusColor(dispute.status), color: 'white' }} className="text-sm px-4 py-2" data-testid="dispute-status">
            {dispute.status}
          </Badge>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* Dispute Overview */}
            <Card className="p-6" style={{ backgroundColor: COLORS.background }} data-testid="dispute-overview">
              <h2 className="text-lg font-semibold mb-4 flex items-center gap-2" style={{ color: COLORS.primary }}>
                <AlertTriangle className="w-5 h-5" /> Dispute Overview
              </h2>
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <p className="text-xs uppercase mb-1 font-medium" style={{ color: COLORS.subtext }}>Dispute ID</p>
                  <p className="font-mono text-sm" style={{ color: COLORS.text }}>{dispute.dispute_id}</p>
                </div>
                <div>
                  <p className="text-xs uppercase mb-1 font-medium" style={{ color: COLORS.subtext }}>Dispute Type</p>
                  <p className="text-sm capitalize" style={{ color: COLORS.text }}>{dispute.dispute_type?.replace(/_/g, ' ') || 'General'}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm" style={{ color: COLORS.text }}>{new Date(dispute.created_at).toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-xs uppercase mb-1 font-medium" style={{ color: COLORS.subtext }}>Raised By</p>
                  <p className="text-sm" style={{ color: COLORS.text }}>{dispute.raised_by_email || dispute.raised_by || 'Unknown'}</p>
                </div>
                {transaction && (
                  <div>
                    <p className="text-xs uppercase mb-1 font-medium" style={{ color: COLORS.subtext }}>Transaction</p>
                    <Link to={`/admin/transaction/${transaction.transaction_id}`} className="text-sm hover:underline" style={{ color: COLORS.info }}>
            ...
            <Card className="p-6" style={{ backgroundColor: COLORS.background }}>
              <h2 className="text-lg font-semibold mb-4 flex items-center gap-2" style={{ color: COLORS.primary }}>
                <MessageSquare className="w-5 h-5" /> Dispute Reason & Description
              </h2>
              <div className="space-y-4">
                <div>
                  <p className="text-xs uppercase mb-1 font-medium" style={{ color: COLORS.subtext }}>Reason</p>
                  <p className="text-sm" style={{ color: COLORS.text }}>{dispute.reason || dispute.dispute_type || 'No reason provided'}</p>
                </div>
                <div>
                  <p className="text-xs uppercase mb-1 font-medium" style={{ color: COLORS.subtext }}>Full Description</p>
                  <div className="p-4 rounded" style={{ backgroundColor: COLORS.section }}>
                    <p className="text-sm whitespace-pre-wrap" style={{ color: COLORS.text }}>{dispute.description || dispute.details || 'No description provided'}</p>
                  </div>
                </div>
              </div>
            </Card>
            ...
            {((dispute.evidence_photos && dispute.evidence_photos.length > 0) || (dispute.evidence && dispute.evidence.length > 0)) && (
              <Card className="p-6" style={{ backgroundColor: COLORS.background }} data-testid="evidence-section">
                <h2 className="text-lg font-semibold mb-4 flex items-center gap-2" style={{ color: COLORS.primary }}>
                  <ImageIcon className="w-5 h-5" /> Evidence Photos ({(dispute.evidence_photos || dispute.evidence || []).length})
                </h2>
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 rounded border" style={{ borderColor: COLORS.border }}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium" style={{ color: COLORS.text }}>{transaction?.buyer_name}</span>
                  </div>
                ...
                <div className="p-4 rounded border" style={{ borderColor: COLORS.border }}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-medium" style={{ color: COLORS.text }}>{transaction?.seller_name}</span>
                  </div>
                  <p className="text-sm" style={{ color: COLORS.text }}>
            ...
            {transaction && (
              <Card className="p-6" style={{ backgroundColor: COLORS.background }}>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <p className="text-xs uppercase mb-1 font-medium" style={{ color: COLORS.subtext }}>Amount</p>
                    <p className="text-lg font-bold" style={{ color: COLORS.green }}>R {transaction.item_price?.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase mb-1 font-medium" style={{ color: COLORS.subtext }}>Item</p>
                  ...
                  <div>
                    <p className="text-xs uppercase mb-1 font-medium" style={{ color: COLORS.subtext }}>Buyer</p>
                    <p className="text-sm" style={{ color: COLORS.text }}>{transaction.buyer_name} ({transaction.buyer_email})</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase mb-1 font-medium" style={{ color: COLORS.subtext }}>Seller</p>
                    <p className="text-sm" style={{ color: COLORS.text }}>{transaction.seller_name} ({transaction.seller_email})</p>
                  </div>
                </div>
                <div className="mt-4">
            ...
            {/* Dispute Timeline */}
            <Card className="p-6" style={{ backgroundColor: COLORS.background }} data-testid="dispute-timeline">
              <h2 className="text-lg font-semibold mb-4 flex items-center gap-2" style={{ color: COLORS.primary }}>
              ...
              <div className="space-y-4">
                <div className="flex gap-4">
                  ...
                  <div className="pb-4">
                    <p className="text-sm font-medium" style={{ color: COLORS.text }}>Dispute Opened</p>
                ...
                {dispute.admin_notes && typeof dispute.admin_notes === 'string' && (
                  <div className="flex gap-4">
                    <div className="flex flex-col items-center">
                ...
                {dispute.resolution && (
                  <div className="flex gap-4">
                    ...
                    <div>
                      <p className="text-sm font-medium" style={{ color: COLORS.text }}>Dispute Resolved</p>
                      <p className="text-xs" style={{ color: COLORS.subtext }}>
          ...
          <div className="space-y-6">
            ...
            <Card className="p-6" style={{ backgroundColor: COLORS.background }} data-testid="admin-actions-panel">
              ...
              <div className="space-y-3">
                <Button 
                  onClick={() => openConfirmModal('release_to_seller', 'Release Funds to Seller', `Release funds to ${transaction?.seller_name}? This resolves the dispute in favor of the seller.`)}
                  disabled={actionLoading || dispute.status === 'resolved'}
                  className="w-full text-white justify-center"
                  style={{ backgroundColor: COLORS.green }}
                  data-testid="release-to-seller-btn"
                >
                ...
                <Button 
                  onClick={() => openConfirmModal('refund_buyer', 'Refund Buyer', `Refund ${transaction?.buyer_name}? This resolves the dispute in favor of the buyer.`)}
                  disabled={actionLoading || dispute.status === 'resolved'}
                  className="w-full text-white justify-center"
                ...
                <Button 
                  ...
                  data-testid="request-buyer-info-btn"
                >
                ...
                <Button 
                  ...
                  className="w-full text-white justify-center"
                  style={{ backgroundColor: COLORS.info }}
                  data-testid="request-seller-info-btn"
                >
                  <HelpCircle className="w-4 h-4 mr-2" />
                ...
                <Button 
                  onClick={() => openConfirmModal('resolve_dispute', 'Resolve Dispute', 'Mark this dispute as resolved? Make sure to add resolution notes.')}
                  disabled={actionLoading || dispute.status === 'resolved'}
                  className="w-full text-white justify-center"
                  style={{ backgroundColor: COLORS.green }}
              ...
              <div className="mt-6 pt-6" style={{ borderTop: `1px solid ${COLORS.border}` }}>
                ...
                <Textarea 
                  ...
                  placeholder="Add notes about this dispute or action..."
                  rows={3}
                  className="mb-3"
                  style={{ borderColor: COLORS.border }}
                ...
                />
                <Button 
                  onClick={() => openConfirmModal('add_note', 'Add Note', 'Save this admin note to the dispute?')}
            ...
            {dispute.admin_notes && (
              <Card className="p-6" style={{ backgroundColor: COLORS.background }}>
                <h3 className="text-sm font-semibold mb-3" style={{ color: COLORS.primary }}>Admin Notes</h3>
                <div className="p-3 rounded text-sm" style={{ backgroundColor: COLORS.section }}>
                  <p style={{ color: COLORS.text }}>
                    {typeof dispute.admin_notes === 'string' 
                      ? dispute.admin_notes 
                      : JSON.stringify(dispute.admin_notes)}
                  </p>
                </div>
              </Card>
            )}
