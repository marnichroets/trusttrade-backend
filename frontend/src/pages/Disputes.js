...
/**
 * Collapse a dispute's raw status + review_status + appeal flags into one of the
 * simple labels shown on each row.
 */
function deriveStatusBadge(dispute) {
  const status = (dispute.status || '').toLowerCase();
  const review = (dispute.review_status || '').toLowerCase();

  if (status.includes('resolved')) return { label: 'Resolved', color: V.success };
  if (status.includes('appeal') || dispute.buyer_appealed || dispute.seller_appealed) {
    return { label: 'Appealed', color: V.warn };
  }
  if (review.includes('ai_recommended') || review.includes('ai_auto')) {
    return { label: 'AI Recommended', color: V.accent };
  }
  if (review.includes('manual')) return { label: 'Manual Review', color: V.error };
  return { label: 'Open', color: V.sub };
}
          ...
          {disputes.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {disputes.map(dispute => {
                const badge = deriveStatusBadge(dispute);
                return (
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    onClick={() => navigate(`/dispute/${dispute.dispute_id}`)}
                    onKeyDown={(e) => { if (e.key === 'Enter') navigate(`/dispute/${dispute.dispute_id}`); }}
                    style={{
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, color: V.accent, fontFamily: V.sans }}>
