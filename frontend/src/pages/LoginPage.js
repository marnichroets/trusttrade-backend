...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Error from location state (internal navigation)
    ...
    // Error from query param (redirect from backend OAuth failure)
    const params = new URLSearchParams(location.search);
    const qError = params.get('error');
    if (qError) {
      const msg = qError === 'google_auth_denied'
        ...
        <Card className="shadow-premium-lg border-slate-200">
          ...
          <CardContent className="pt-2 pb-6">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 h-10 border border-slate-200 rounded-md bg-white hover:bg-slate-50 transition-colors text-sm font-medium text-slate-700 mb-4 disabled:opacity-60"
            >
              {googleLoading ? (
