...
function TransactionsList() {
  ...
  const renderTransactionRow = (t) => {
    const uiState = resolveEscrowUiState(t);
    const expiry = getPendingPaymentExpiry(t);
    const paymentLabel = uiState.state === 'EXPIRED'
      ? 'Expired'
      : uiState.state === 'FUNDED'
        ? 'Awaiting payment'
        : t.payment_status;
    const paymentHint = uiState.state === 'EXPIRED'
      ? 'Transaction expired due to no payment'
      : uiState.state === 'FUNDED'
        ? expiry.expiresInLabel
        : null;
    ...
    return (
      ...
      >
        <td style={{ ...cell, fontFamily: V.mono, fontSize: 11, color: V.sub }}>
          {t.transaction_id.substring(0, 12)}â€¦
        </td>
        <td style={cell}>{t.buyer_name}</td>
        <td style={cell}>{t.seller_name}</td>
        <td style={{ ...cell, maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={t.item_description}>
          {t.item_description}
        </td>
        ...
        <td style={cell}>
          <span style={{
            ...
            fontFamily: V.mono,
            color: STATUS_COLOR[paymentLabel] || (uiState.state === 'EXPIRED' ? V.sub : V.warn),
            background: `${STATUS_COLOR[paymentLabel] || (uiState.state === 'EXPIRED' ? V.sub : V.warn)}18`,
            border: `1px solid ${STATUS_COLOR[paymentLabel] || (uiState.state === 'EXPIRED' ? V.sub : V.warn)}40`,
          }}>
            {paymentLabel}
          </span>
          {paymentHint && (
            <div style={{ marginTop: 5, fontSize: 11, color: uiState.state === 'EXPIRED' ? V.sub : V.warn, fontFamily: V.mono }}>
              {paymentHint}
            </div>
          )}
        </td>
